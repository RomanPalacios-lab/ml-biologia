...
$$C_{p} = \frac{1}{n}(RSS + 2d\hat{\alpha}_^{2})$$ 
Donde $\hat{\alpha}_^{2})$ es el estimado del error de varianza $\epsilon$ asociado a cada medida de respuesta y *d* es el número total de parámetros usados. Este método añade una penalidad de $2d\hat{\alpha}_^{2})$ para el RSS de entrenamiento, debido a que el error de entrenamiento tiene a subestimar el error de prueba. *Nota: esta penalidad incrementa a medida que el número de predictores en el modelo aumenta* 
